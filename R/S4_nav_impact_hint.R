...
num_to_select <- 10000
idx <- sample(vcount(hint_net) * vcount(hint_net), num_to_select)
src <- ((idx - 1) %% vcount(hint_net)) + 1
trg <- floor((idx - 1) / vcount(hint_net)) + 1
pin_routeing <- greedy_route_packets(hint_net, coords, src, trg)
...
eff <- eff %>% 
  map(impact_on_nav, coords, hint_net, st = 500, epochs = 10, faulty = 20)
...
save(ref_eff, ref_hop, eff, imp, file = "results/S_nav_impact_hint.RData")
