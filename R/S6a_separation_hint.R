...
for(i in 1:length(diseases)) {
  for(j in i:length(diseases)) {
    if(i != j) {
      idx.i <- which(V(hint_net)$name %in% diseases[[names(diseases)[i]]]$geneId)
      idx.j <- which(V(hint_net)$name %in% diseases[[names(diseases)[j]]]$geneId)
...
save(dH.matrix, file = "results/S_separation_hint.RData")
