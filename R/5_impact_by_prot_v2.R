...
enr_freq <- fun_enrich(names(freq_faul), V(hPIN)$name, benjamini = TRUE)
p_freq <- plot_fun_enrich(enr_freq, benjamini = TRUE, char_per_line = 40)

enr_infreq <- fun_enrich(names(infreq_faul), V(hPIN)$name, benjamini = TRUE)
p_infreq <- plot_fun_enrich(enr_infreq, benjamini = TRUE, char_per_line = 40)

sym_freq <- V(hPIN)$symbol[V(hPIN)$name %in% names(freq_faul)]
sym_infreq <- V(hPIN)$symbol[V(hPIN)$name %in% names(infreq_faul)]
...
readr::write_tsv(freq_tab,path = "data/S_freq_faulty.tsv")

save(df, p_dt, pval_fda, pval_pot, freq_tab, 
     enr_freq, p_freq, enr_infreq, p_infreq, file = "results/nav_imp_drugs.RData")
